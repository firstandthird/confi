...
const load = (options, callback) => {
  ...
  async.autoInject({
    ...
    defaultConf(opts, done) {
      readConfig(opts.configFile || opts.path, 'default', opts.configFile, done);
    },
    envConf(opts, done) {
      readConfig(opts.path, opts.env, false, done);
    },
    userConf(opts, done) {
      if (opts.user) {
        const userConfPath = (typeof opts.userPath === 'string') ? opts.userPath : opts.path;
        return readConfig(userConfPath, opts.user, false, done);
      }
      return done(null, {});
    },
    ...
    config(defaultConf, packageOpts, envConf, userConf, envVarConf, opts, done) {
      const config = {};
      aug(true, config, defaultConf, packageOpts, envConf, userConf, envVarConf, opts.context);
      config.env = opts.env;
      config.CWD = cwd;
