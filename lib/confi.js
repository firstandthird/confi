...
const load = (options) => {
  ...
  if (options && options.package) {
    let includePath = path.join(process.cwd(), 'package.json');
    if (typeof options.package === 'string') {
      if (options.package.indexOf('package.json') > -1) {
        includePath = path.join(process.cwd(), options.package);
      } else {
        includePath = path.join(process.cwd(), options.package, 'package.json');
      }
    }
    const foreignPackage = require(includePath);
    packageOpts = foreignPackage.confi ? foreignPackage.confi : {};
  }
