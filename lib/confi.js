...
const defaults = {
  ...
  context: {},
  helpers: {
    _split(input, splitOn) {
      splitOn = splitOn || ',';
      return input.split(splitOn);
    },
    _get(input, defaultVal) {
      if (input) {
        return input;
      }
      return defaultVal;
    },
    _if (checkValue, ifTrueValue, ifFalseValue) {
      if (checkValue) {
        return ifTrueValue;
      }
      return ifFalseValue;
...
const load = (options, callback) => {
  ...
  async.autoInject({
    ...
    config(defaultConf, packageOpts, envConf, userConf, urlConf, envVarConf, fileConf, opts, done) {
      const config = {};
      return done(null, aug(config, defaultConf, packageOpts, envConf, fileConf, urlConf, userConf, envVarConf, opts.context));
    }
  }, (err, results) => {
    if (err) {
      return callback(err);
