...
const load = (options) => {
  ...
  if (opts.userConfig) {
    ...
    const userConfigPath = path.join(os.homedir(), '.confi', `${projectName}.yaml`);
    const content = parseDir.sync(userConfigPath);
    if (content[0] && content[0].contents) {
      userConf = content[0].contents;
    }
  }
